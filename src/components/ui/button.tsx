...
const buttonVariants = cva(
  'inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-bold tracking-wide ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 uppercase',
  {
    variants: {
      variant: {
        default: 'bg-white text-black b-slate-200 b-2 b-b-4 active:b-b-2 hover:bg-slate-100 text-slate-500',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, asChild = false, block = false, ...props }, ref) => {
  const Comp = asChild ? Slot : 'button'
  return <Comp className={`${cn(buttonVariants({ variant, size, className }))} ${block ? 'block w-full' : ''}`} ref={ref} {...props} />
})
